  ...
  return (
    ...
    <TextInput placeholderTextColor="#BABFC8" style={{
     ...
     <Image source={searchIcon} style={{
      ...
      <View>
      <FlatList
        data={cityLists?.citys.slice(0,5)}
        renderItem={({item}) => (
          <View style={style.imageContainerDance} onTouchEnd={() => navigation.navigate('classes-list', {city: item.cityName})}>
            <Image style={{alignSelf: 'center',width:55,height:55,}} source={{uri: `${API_URL_IMAGE}/${item.profileImage}`}} />
        ...
        style={{ position: 'relative', top: '-15%' }}
          ...
          renderItem={({item}) => (
            <View
              ...
              <Text style={{...style.imageTitleDance, top:-6, color:'#FFF'}}>{item?.name}</Text>
            </View>
          )}
          numColumns={4}
        />
